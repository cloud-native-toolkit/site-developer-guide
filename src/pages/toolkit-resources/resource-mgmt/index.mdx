...
- **Two development environments**: Two development teams, each has its own development environment. Each development environment has a corresponding resource group, in this example named `team1` and `team2`. The first has a Kubernetes cluster while the second has an OpenShift cluster.
...
These are not all the access groups you'll ever need because even finer-grained access management is possible, but this is a pretty good start that will work well for typical accounts.
