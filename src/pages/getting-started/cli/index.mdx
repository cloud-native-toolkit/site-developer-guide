...
1. Set the current namespace/project
    ...
    **For Kubernetes:**
    ...
    ```
2. Configure the service account `pipeline` if it doesn't exist, on OpenShift 4 the operator takes care of this you can skip.
    ```shell script
    oc create serviceaccount pipeline
    oc adm policy add-scc-to-user privileged -z pipeline
    oc adm policy add-role-to-user edit -z pipeline
    ```
3. Clone the tasks from the `tools` namespace into the current namespace
    ```shell script
    kubectl get tasks -o json -n tools | \
      ...
      kubectl apply -f -
    ```
4. List the available pipeline templates in the `tools` namespace. Select the one to use for your project.
    ```shell script
    kubectl get pipelines -n tools
    ```
5. Clone the selected pipeline from the `tools` namespace into the current namespace
    ```shell script
