...
Creates a namespace (if it doesn't already exist) in the current cluster and prepares it for use:
- Copies in pull secrets from the default namespace
- Copies config maps and secrets from the template namespace
- Adds pull secrets to a service account

Optionally, the namespace command can also set up the Jenkins environment and/or Tekton environment. The command will install Jenkins into the namespace (only available on OpenShift) and set up the Jenkins serviceAccount. Or will copy the available Tasks and Pipelines from the template namespace.

Command flags:
- `-t` -- The template namespace; the default is `tools`. When run against OpenShift, this command will actually create a `project` which  result in the creation of a namespace as well.
- `-z` -- The name of the service account. The default is `default`.
- `--jenkins` -- Set up the Jenkins environment.
- `--tekton` -- Set up the Tekton environment.
