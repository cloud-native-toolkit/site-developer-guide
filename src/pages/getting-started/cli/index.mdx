...
To use the <Globals name="igccli" />, first:
- Install the <Globals name="igccli" /> in your terminal's path
- Use the <Globals name="iccli" /> to log your terminal in to <Globals name="ic" />
- Use the cluster CLI to log your terminal in to the <Globals name="kube" /> or <Globals name="ocp" /> cluster

The <Globals name="iccli" /> uses those logged in sessions to connect to <Globals name="ic" /> and the cluster.
...
**Warning**: If you have installed the **<Globals name="igccli" />** prior to
 `v0.4.0`, you must uninstall it and follow the installation steps below. The
  uninstall command: `npm rm -g @garage-catalyst/ibm-garage-cloud-cli`

</InlineNotification>

Follow these steps to install the <Globals name="igccli" />:
...
- Log into your cluster:
    ...
    For <Globals name="kube" />:
    ```bash
    ibmcloud ks cluster config --cluster CLUSTER
    ```

    For <Globals name="ocp" />:
    ```bash
