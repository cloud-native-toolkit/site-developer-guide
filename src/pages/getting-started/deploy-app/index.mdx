...
First, create your project namespace. (See the [IGC CLI](/getting-started/cli) for details about how the `namespace` command works.)

```bash
igc namespace dev-bw --jenkins --tekton
```

Now create a pipeline for your repo. You need to specify whether your CI pipeline will run in Tekton (`--tekton`) or Jenkins (`--jenkins`).
