...
### Option A-1: Select from account's VLAN List (Advanced)
...
### Option A-2: Install and use the Developer Tools CLI (Simpler)
...
- Edit the `terraform.tfvars` file to set the properties' values.
    ```bash
    vi ./terraform/settings/terraform.tfvars
    ```
    These values should look something like the example below. For example, let's say your resource group is called `catalyst-team`, 
    your region is `us-south`, your zone is `dal10`, your zone's private VLAN is `2372`, and your zone's public VLAN is `1849`. 
    Then `terraform.tfvars` would look like this:
    ```terraform
...
### Option B-1: Optional settings

Four other properties are used to specify optional settings.

- To specify the name of the new cluster, or to specify the name of an existing cluster:
...
- To specify installing the tools in an existing cluster:
...
- To specify the type of cluster, Kubernetes or OpenShift:
...
- To specify whether the Postgres server already exists (which contains information about the cluster):
