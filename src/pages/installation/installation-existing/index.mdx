...
provisioned by the scripts. You also need to remember to remove any IBM Cloud services that were previously
created. Use the helper script `terraform/scripts/destroy-service.sh` to help automate the deletion You can also run it
to remove all the services beside **PostgreSQL** `./destroy-services.sh post`
