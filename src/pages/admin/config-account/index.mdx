...
First, we'll upgrade the service plan for the [image registry](/getting-started-day-1/image-registry/) so that is has more capacity.
...
- Log into the IBM Cloud CLI [using the functional ID key](https://cloud.ibm.com/docs/account?topic=account-federated_id) created by the account owner
    ```bash
...
- Create a new [access group](https://cloud.ibm.com/docs/account?topic=account-groups), name it something like `<resource_group>-ADMIN` (all capital letters)
...
- Create a new [access group](https://cloud.ibm.com/docs/account?topic=account-groups), name it something like `<resource_group>-USER` (all capital letters)
