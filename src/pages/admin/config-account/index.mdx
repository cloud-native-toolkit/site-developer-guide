...
Seventh, create an access group to enable developers to access the <Globals name="env" />. This can be done later, after the environment is created, either by running a script or using the console. Also, add the developers (the users who will use the environment) to this group.

- Use the dev policies script

    - Create a new [access group](https://cloud.ibm.com/docs/iam?topic=iam-account_setup), name it something like `<resource_group>-USERS` (all capital letters)
    - Run the script `./terraform/scripts/acp-users`, which will add the necessary policies to the access group
