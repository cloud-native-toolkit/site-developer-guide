...
Fifth, create an access group to grant the necessary permissions for installing a <Globals name="env" />. Do this by running a script or using the console. Also, add the environment administrator(s) (who is the user who will run the scripts to create the environment) to this group.
...
Sixth, create an access group to enable developers to access the <Globals name="env" />. This can be done later, after the environment is created, either by running a script or using the console. Also, add the developers (the users who will use the environment) to this group.
