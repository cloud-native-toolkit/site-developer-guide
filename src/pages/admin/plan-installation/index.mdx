...
- Create an `ACCT-MGR-IAM` access group using the `acp-mgr` script
- Add a functional ID, configured using the `acp-mgr-iaas` script, with API keys for the account managers
...
**Note**: IBM Cloud has multiple [account management services](https://cloud.ibm.com/docs/iam?topic=iam-account-services), in addition to IAM: Billing, License and entitlement, Support center, etc. An easy way to grant access to these individually is to create an access group for each: ACCT-MGR-BILLING, ACCT-MGR-LICENSE, ACCT-MGR-SUPPORT. The group created below only has IAM capabilities, so a good name for it is ACCT-MGR-IAM.

</InlineNotification>

To create the access group for the account managers:
- Create a new [access group](https://cloud.ibm.com/docs/iam?topic=iam-account_setup), name it something like `ACCT-MGR-IAM` (all capital letters) (or name it after your account)
...
- Run the script `./terraform/scripts/acp-mgr-iaas.sh` for the functional ID's user, which adds the necessary permissions to the user
