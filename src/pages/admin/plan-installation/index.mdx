...
**Note**: The **account owner** creates this access group for account managers. See [Plan Installation > Roles](/admin/plan-installation#roles) for the overview of the roles involved.
...
### Infrastructure permissions for account managers

Account managers need the permissions to create and manage IaaS resources required by a <Globals name="env" />. Permissions for classic infrastructure (fka Softlayer) cannot be added to an access group, only to a user.

For each user added to the account manager access group:
- Run the script `./terraform/scripts/acp-mgr-iaas.sh`, which adds the necessary permissions to the user

The script adds infrastructure permissions to:
- Create VLANs
- Create Kubernetes Service clusters (e.g. create virtual servers, storage, and networking)
- Manage Kubernetes Service clusters

When an account manager [creates an API key for the Kubernetes Service](/admin/config-account#resource-group), IKS will use that key and the account manager's infrastructure permissions to create the IaaS resources for the cluster.

Whenever a user is removed from the account manager access group, also remove the user's infrastructure permissions.
