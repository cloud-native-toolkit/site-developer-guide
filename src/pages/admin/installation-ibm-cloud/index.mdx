...
The Developer Tools environment is installed by an environment administrator, who will run the scripts to create the environment in an IBM Cloud account. The scripts will run as the environment administrator's user, so the user needs the permissions described in [Plan Installation](/admin/plan-installation). These instructions explain how to configure and run the Terraform [infrastructure-as-code](/admin/iasc) (IasC) scripts to create the Developer Tools environment.
...
The environment administrator must be able to log into IBM Cloud, specifying the account, region, and resource group. That is effectively what the scripts will do when they run, using the environment administrator's API key.

Optional: To help confirm that the account has the resources the scripts will require, the environment administrator may  log into the account view the resources:
