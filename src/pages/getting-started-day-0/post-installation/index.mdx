...
Generate new passwords for **SonarQube** and update the `sonarqube-access` secret in the `tool` namespace

### Log into each of the tools
...
### Create the namespace/project for the Development Team's CI environment
...
the `dev` namespace created in the previous step.
...
**Note:** Use the project/namespace from the previous two steps when deploying the app 
