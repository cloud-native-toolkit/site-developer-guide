---
title: Install the Toolkit using Iteration Zero.
...
Steps to install the <Globals name="shortName" /> for running Openshift or Kubernetes clusters.
...
3. Update the value for the `ibmcloud.api.key` property in `credentials.properties` with your <Globals name="ic" /> API key
    ...
    **Note:** The API key should have been set up during [plan installation](/getting-started/prereqs#create-an-ibm-cloud-api-key).
