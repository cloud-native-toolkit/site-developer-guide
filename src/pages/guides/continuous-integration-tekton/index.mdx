...
With Tetkon enabled and your default `Tasks` and and `Pipelines` installed in
 to the `dev` namespace. You can now configure you applications to be built, packaged, tested and deployed to your OpenShift or Kubernetes development cluster.
...
The `IBM Garage CLI` has been created to automate manual steps where it is
 deemed necessary. When it comes to configuring **Tekton** there are a number
  of key steps that developers can performing manually,  often with complex
   manual steps the developer can introduce errors caused by mistakes. This
