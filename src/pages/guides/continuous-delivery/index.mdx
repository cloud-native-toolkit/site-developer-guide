...
personal access token, and the branch to put in the secret.
    ...
    **What just happened?**
    ...
    The `git-secret` command creates a secret in kubernetes containing the url, username, password, and branch information
    for a git repo into the given namespace. In the command above, we provided `gitops-cd-secret` for the secret name. You
    can verify the secret was created by running 
