...
personal access token, and the branch to put in the secret.
    ...
    **Note:** The namespace (`dev-{initials}`) should match the namespace where the pipeline is running. The value provided
    for `branch` is the one the pipeline will use to when commiting changes to trigger the CD pipeline. `test` is the
    recommended value for the branch field.
