...
- Before you starting understanding how to look at your logs, make sure you have [deployed an app](/getting-started/deployapp) into your development cluster. each of the template apps have a logging framework included, this _Git it a Try_ will use the [template-node-typescript](/starterkits/starterkittemplates) example.
...
You can now see just the logs for your app.

### Add some logging to your app

All the common logging frameworks support the common log statement that a developer will use in their code `info`, `debug`, `warn` and `fatal`. It is best practice to use add log statements to correctly help operations teams and monitoring services to help identify possible issues quickly.app

- Add the following `debug` statement to the `greetings` implementation in your app.app
    ...
    ```
- to add `LOG_LEVEL` settings to you app
