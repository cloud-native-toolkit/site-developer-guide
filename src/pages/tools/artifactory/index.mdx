...
- [**Maven**](https://maven.apache.org): Builds Java artifacts (such as Jar, War, Ear, etc.) and projects stored in [Maven repositories](https://maven.apache.org/tools/introduction/introduction-to-repositories.html) such as [Maven Central](https://maven.apache.org/repository/index.html)
...
- [**PIP**](https://pypi.org/project/pip/): Installs Python packages from [index repositories](https://packaging.python.org/tools/hosting-your-own-index/) such as the [Python Package Index (PyPI)](https://pypi.org/)
...
Note: Be sure to [set up Artifactory](/admin/artifactory-setup/) before using it in the Development Tools environment, if you are using external Artifactory that was not set up as part of the toolkit installation.

The <Globals name="env" /> will eventually be extended to store a number of artifact types in Artifactory. Thus far, the CI and CD pipelines exchange two types of artifacts: Docker images and Helm charts. The CI pipeline ([Jenkins](/tools/jenkins), [Tekton](/tools/tekton), etc.) builds these artifacts and ArgoCD deploys them. To store and share the artifacts, the pipeline uses two repositories:
- **Docker images**: The [Developer Tools Image Registry](/tools/ibm-cloud-container-registry)
- **Helm charts**: A Helm repository in Artifactory

The [<Globals name="templates" />](/starterkits) have also been configured to store their Helm charts in Artificatory.

Artifactory is part of the <Globals name="env" />'s complete CI/CD solution:

<AnchorLinks small>
  <AnchorLink to="/getting-started-day-1/continuous-integration">Continuous Integration</AnchorLink>
  <AnchorLink to="/getting-started-day-2/continuous-delivery">Continuous Delivery</AnchorLink>
</AnchorLinks>
...
- Use the [<Globals name="dashboard" />](/getting-started-day-1/dashboard) to open the Artifactory dashboard
