...
This video demostrates how to work through the steps below to configure GitOps and Argo CD to deploy an application to test and staging namespaces in an OpenShift cluster.
...
1. Clone the project to your machine and navigate into the directory
...
1. Copy the helm chart template into application name folder

    ```shell script
    cp -r templates/project-config-helm {APPLICATION_NAME}
    ```

1. Commit the changes and push the test branch up to the repository

    ```shell script
    git add .
    git commit -m 'add helm chart template'
    git push -u origin test
    ```

1. Create a corresponding OCP namespace for `test`

    ```shell script
    igc namespace {TEST_NAMESPACE}
    ```

1. Grant access to service accounts in the new `test` namespace to pull the container image from the `dev` namespace
...
**Note:** Repeat the last 5 steps above for any other envioronments you wish to deploy to such as staging and production.
