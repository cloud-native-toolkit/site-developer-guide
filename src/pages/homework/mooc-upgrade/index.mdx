...
The Dashboard now support OpenShift single sign on and uses the OAuth
 Challenge.
 
 ### CLI
 
-  Install the latest IBM Garage for Cloud Developer Tools CLI, there has been
 some minor bug fixes to address issues seem from the first phase of the MOOC.   
    ```$bash
...
This is generic for all Starter Kit Templates, the templates themselves have been updated so any new apps being based on them will be work fine. 

### Registering you pipeline

- Run the following commands for each app that you deployed. 
    ```bash
...
https://ibm-garage-cloud.github.io/ibm-garage-developer-guide/practical/inventory-part2#using-cd-to-deploy-to-test
...
https://ibm-garage-cloud.github.io/ibm-garage-developer-guide/practical/inventory-part2#securing-the-solution-with-app-id

### Data in Cloudant

You will need to complete this section of the Session 1, Week 2 material to repopulate your database.

https://ibm-garage-cloud.github.io/ibm-garage-developer-guide/practical/inventory-part2#add-a-cloudant-integration-to-your-backend-service

You will need to update the `mappings.json` with the new Cloudant Instance
 credentials

Thanks for all the help and support during the Live Sessions. You can continue to return to the MOOC and walk through the self paced material.
