...
This guide assumes you are using a Developer Tools environment that has already been installed, and that Cloud Pack for Applications has already been installed in the environment.
...
When the code is pushed to the server repo, the pipeline will get triggered.
...
- Find your project, `dev-{initials}`
...
- In the Create Webhook form, enter the following values:
    ...
    - **Pipeline**: Select `nodejs-express-build-deploy-pipeline-fix` (the one with the `-fix` suffix)
...
Configure the local repo to map to the server repo you created earlier and sync them.
...
Click on the PipelineRun to see its details. In its list of Tasks, expand one to see its steps. Select a step to see its details: logs, status, and details.
...
- When the tekton pipeline is done, get the URL of the Knative service
    ```bash
    $ oc get ksvc
