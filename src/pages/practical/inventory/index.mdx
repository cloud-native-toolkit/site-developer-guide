...
6. Open the zip file and copy `api.ts` into a directory in the root of the BFF project named 
`inventory-management-service-client`.

7. Add a `package.json` file in the `inventory-management-service-client` folder with the following contents:

    inventory-management-service-client/package.json
    ```json
    {
      "name": "inventory-management-service-client",
      "version": "1.0.0",
      "dependencies": {
        "bluebird": "^3.7.1",
        "request": "^2.88.0"
...
8. Update the `package.json` in the root of the BFF to include the client as a dependency
    ...
    ```json
    {
      ...
      "dependencies": {
        "inventory-management-service-client": "file:./inventory-management-service-client",
        ...
      },
      ...
    }
...
9. Run `npm install` in the root directory of the BFF to bring down all the project dependencies

#### Create a service implementation that calls the microservice

1. Add a `stock-item-service` config
...
2. Add `stock-items` service that uses the config and the sdk for the microservice
    ...
    import {StockItem, StockItemControllerApi} from 'inventory-management-service-client';
    
    import {StockItemsApi} from './stock-items.api';
    import {StockItemModel} from '../models';
    import {StockItemServiceConfig} from '../config/stock-item-service.config';
