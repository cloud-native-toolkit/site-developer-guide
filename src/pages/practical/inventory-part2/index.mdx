...
- Find the app id binding secret by running the following command from a terminal where the kubernetes context
has been set up (i.e. `oc login` has been done)
...
- Open `src/main/java/com/ibm/inventory_management/services/StockItemService.java` and add the `@Primary` annotation. The file
