...
<AnchorLinks>
  ...
  <AnchorLink to="#verify-deployed-microservice-app">Verify deployed microservice app</AnchorLink>
</AnchorLinks>
...
- Repeat the steps above to add the Appsody deployment manifest and edit its namespace to `dev-{initials}`

- Also in `app-deploy.yaml`, add an environment variable to link the frontend service to the backend service on port 3001
    ```yaml
    spec:
      env:
        - name: APPSODY_BACKEND_DEFAULT_URL
          value: http://appsody-backend-{initials}:3001/
    ```
...
## Verify deployed microservice app
