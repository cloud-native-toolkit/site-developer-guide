...
- Test the app
    ...
    - Confirm the app is working as expected with the DB2 data by clicking the `Flowers` link

    ![PlantsByLiberty](images/liberty-pbw.jpg)
...
That's it. You now have this application running on Liberty in a Docker container. For next steps, you would likely push this image to a Red Hat OpenShift image registry and deploy the application using the Open Liberty operator. If the client is interested in adding CI/CD automation, you may also go back to the source code and automate CI and then automate deployment using CD.
