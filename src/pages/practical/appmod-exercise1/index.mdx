...
For the purposes of this lab exercise you will run a simulation of the WebSphere ND environment on your laptop using a pre-configured Docker image that contains PlantsByWebSphere. You will then use IBM Cloud Transformation Advisor that we have deployed for you in your MOOC OCP Cluster to create a IBM Cloud Transformation Advisor Data Collector to analyze the WebSphere ND environment and create the artifacts you need to run the same PlantsByWebSphere application on Liberty in a Docker container on your laptop.
...
That's it. You now have this application running on Liberty in a Docker container which is sufficient for this exercise. In a real MVP you would now need to perform the following tasks which are out of scope for this exercise:
- Automate the build process for the application code. This would require you to load the source code in to Git and configure a build tool such as Maven to compile the application and create the EAR/WAR file. The steps to achieve this are documented [here](https://medium.com/@davemulley/modernizing-the-build-process-of-existing-java-ee-applications-with-maven-c8e9b43b6876)

- Externalize configuration parameters such as the database IP, Port and UserID/Password to allow the same image to be deployed in different environments. Refer to the list of minimum changes in [this article](https://medium.com/@davemulley/what-are-the-minimum-changes-i-should-make-to-my-websphere-application-before-deploying-to-a8418ca8f871)
