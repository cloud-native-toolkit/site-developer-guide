...
- The DevOps process is compose of Continous Integration (CI) with OpenShift Pipelines (Tekton) and Continous Deployment (CD) with GitOps engine ArgoCD
...
1. (Optional) Analyze the application using the following guide [Modernizing runtimes with Liberty](https://ibm-cloud-architecture.github.io/modernization-playbook/applications/liberty/liberty-analyze)
    ...
    - Review the CustomerOrderServicesApp.ear analisys
    - The migration path files have being deploy to git for this lab.
...
1. Create a pipeline for the application
    ...
    - The Java bin path is not located in the root of the git repository instead in `CustomerOrderServicesApp/target`
...
1. Register the Application in ArgoCD to deploy using GitOps
    ...
    - Click **NEW APP**
    - Application Name: ${TOOLKIT_PROJECT}-websphere-liberty (ie project1-websphere-liberty)
